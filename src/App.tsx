...
const PHASES_CONFIG = [
  ...
  },
  {
    ...
    outils: ["dialogue","dossier","onisep","tampon"],
    annonce: {
...
const ELEVES = [
...

  {
    ...
    piegeType:"projet_irrealiste",
    raisonnement:"PIÈGE : Luc veut être pilote de ligne mais évoque lui-même qu'il «décroche souvent». L'ATPL exige un excellent niveau. La voie réaliste : Bac Pro Aéronautique d'abord, pour construire ses compétences.",
...

  {
    ...
    raisonnement:"Léa dit médecine mais l'avis du prof signale qu'elle ne supporte pas la vue du sang. 2nde GT est correct, mais l'orientation vers psychologie ou travail social serait plus adaptée que médecine.",
    piegeDescription:"⚠️ NUANCE : Léa dit 'médecine' mais l'avis du prof révèle une forte sensibilité au sang. 2nde GT est juste, mais noter l'incompatibilité avec médecine chirurgicale.",
    repliques:[
...

  {
    ...
    resumeVisible: "15 ans · Confiant · Parle bien en sciences",
    moyenne:14.0,
    orientationsValides:["pro"],
    piegeType:"absenteisme",
    raisonnement:"PIÈGE : 14/20 semble bon pour la 2nde GT, mais les absences en sciences physiques révèlent que Karim évite les matières difficiles. Son profil pratique et son intérêt pour l'optique pointent vers le Bac Pro Optique-Lunetterie.",
    piegeDescription:"⚠️ PIÈGE : La moyenne de 14/20 masque un absentéisme sélectif en sciences. Son aisance à l'oral et son intérêt pour les métiers de la vue orientent vers le Bac Pro Optique-Lunetterie.",
    repliques:[
      "Bonjour. J'ai une bonne moyenne, ça devrait bien se passer.",
      "J'aime beaucoup les sciences et parler avec les gens.",
      "Mon cousin est opticien, je trouve ça fascinant.",
      "Les sciences physiques... j'y vais moins souvent.",
    ],
    questions:[
      { q:"Ton projet professionnel ?",    r:"Opticien ou quelque chose dans la santé visuelle. Mon cousin a son cabinet.", emoji:"🎯" },
      { q:"Combien d'années d'études ?",   r:"3-4 ans. Une formation pratique me plairait bien.", emoji:"📅" },
      { q:"Tes matières préférées ?",      r:"La biologie et l'anglais. Les sciences physiques... j'y vais moins souvent.", emoji:"📖" },
    ],
    dossier: {
      bulletinResume:"Biologie 17/20 · Anglais 15/20 · Français 14/20 · Sciences physiques 9/20* · Maths 11/20",
      avisProf:"Karim est brillant en biologie et très à l'aise à l'oral. Souvent absent en sciences physiques (notes marquées *). Sa moyenne globale mérite un examen attentif matière par matière.",
      absencesIndice:"* Sciences physiques : note calculée sur peu d'évaluations",
    },
    famille: {
      reponse:"Son père : Karim a 14 de moyenne, on veut la 2nde GT pour lui. Il peut réussir partout.",
      concordance:"contredit",
    },
    pronote: {
      absences:"11 demi-journées d'absence. 4 retards.",
      retards:4,
      absencesParMatiere:"⚠️ Sciences physiques : 7 absences · Biologie : 0 absence · Maths : 1 absence · Anglais : 0 absence",
      alertes:"⚠️ Absences concentrées sur un seul groupe de matières. Aucune absence dans les autres.",
    },
  },
  {
    ...
    raisonnement:"Théo veut le lycée général mais la famille pousse pour qu'il reprenne la boulangerie. 11,5/20 avec un profil gestion : la 2nde GT reste accessible s'il s'accroche — il peut y surmonter les difficultés et s'y épanouir.",
    piegeDescription:"⚠️ DISCORDANCE : Théo veut la 2nde GT mais la famille pousse pour la boulangerie familiale. Son profil éco est cohérent avec la 2nde GT — il peut y surmonter les difficultés en sciences et s'y épanouir.",
    repliques:[
...

  {
    ...
    raisonnement:"Axel veut lui-même le 2nde GT mais sa famille pousse pour le général malgré 9/20. 9/20 avec un profil technique solide : la 2nde Pro (Bac Pro électrotechnique) est la voie où il peut s'épanouir pleinement.",
    piegeDescription:"⚠️ PIÈGE : La famille veut la 2nde GT malgré 9/20. L'intérêt de l'élève prime : Axel peut s'épanouir pleinement en 2nde Pro, filière qui valorise ses vraies compétences techniques.",
    repliques:[
...

  {
    ...
    raisonnement:"13/20 semble correct pour le général, mais Pronote révèle 22 absences en maths. Sans ces absences, la vraie moyenne en maths serait probablement insuffisante. la 2nde GT reste possible mais risquée au vu des absences en maths.",
    piegeDescription:"⚠️ PIÈGE PRONOTE : Mathis a 13/20 mais 22 absences en maths (notes manquantes). Sa vraie maîtrise des maths est inconnue. Le risque de mise en échec au lycée général est réel. La 2nde GT est envisageable si les absences en maths sont résolues, sinon la 2nde Pro peut aussi l'épanouir.",
    repliques:[
...

  {
    ...
    questions:[
      ...
      { q:"Combien d'années d'études ?",      r:"4-5 ans. 2nde GT puis des études en sciences humaines ou droit.", emoji:"📅" },
...
export default function App() {
  ...
  const [resultData, setResultData]         = useState(null); // {correct, newErreurs, nextIdx}
  ...
  const resultPending = useRef(null);
  ...
  const handleStamp = (filiere)=>{
    ...
    const newErreurs = erreurs + (correct ? 0 : 1);
    const nextIdx = eleveIdx + 1;
    setDernierCorrect(correct);
    // Capturer l'élève courant dans resultData pour éviter le re-render stale
    setResultData({ correct, newErreurs, nextIdx, eleveCapture: eleve });
    setTotal(t=>t+1);
    if(correct) setScore(s=>s+1);
    else setErreurs(newErreurs);
    setTimeout(()=>setModalResultat(true), 200);
  };

  // suivant() inliné dans ModalResultat
  ...
  // ── JEU ─────────────────────────────────
  // Indicateur phase suivante (pré-calculé hors JSX pour éviter IIFE)
  const _prochaine = PHASES_CONFIG.find(p=>p.id===phaseActuelle+1);
  const _elevesDansPhase = ELEVES.filter(e=>e.phase===phaseActuelle);
  const _idxDansPhase = eleveIdx - ELEVES.filter(e=>e.phase<phaseActuelle).length;
  const _restants = _elevesDansPhase.length - _idxDansPhase - 1;
  const indicateurPhase = (_prochaine && _restants <= 1) ? (
    <div style={{position:"absolute",bottom:4,right:8,
      fontSize:"9px",color:"rgba(255,255,255,0.4)",
      fontFamily:"Courier New,monospace"}}>
      {"Prochain outil : "}{_prochaine.annonce ? _prochaine.annonce.emoji : ""}{" "}{_restants===0?"après cet élève":("dans "+_restants+" élève")}
    </div>
  ) : null;

  const handleSuivant = ()=>{
    if(!resultData) return;
    const newErreurs = resultData.newErreurs;
    const nextIdx = resultData.nextIdx;
    setModalResultat(false);
    if(newErreurs >= 3 || nextIdx >= ELEVES.length){ setEcran("fin"); return; }
    const prochain = ELEVES[nextIdx];
    if(prochain && prochain.phase > phaseActuelle){
      const nc = PHASES_CONFIG.find(p=>p.id===prochain.phase);
      setPhaseActuelle(nc.id);
      if(nc.annonce){ setEleveIdx(nextIdx); setAnnonce(nc.annonce); return; }
    }
    setEleveIdx(nextIdx);
  };
  ...
  return (
    <div style={{width:"100vw",height:"100vh",overflow:"hidden",
      ...
      {modalResultat && resultData && <ModalResultat eleve={resultData.eleveCapture} choix={null}
        correct={resultData.correct} onSuivant={handleSuivant}
      />}
    </div>
  );
}